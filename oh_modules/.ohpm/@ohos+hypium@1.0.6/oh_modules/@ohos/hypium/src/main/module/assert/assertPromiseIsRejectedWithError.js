...
function matchError(actualValue, expectedValue) {
    ...
    if (expectedValue.length == 1) {
        return {
            pass: false, message: 'When only one parameter, it should be error type or error message.'
        };
    }
    ...
    if (expectedValue.length == 2 && typeof expectedValue[0] === 'function' && expectedValue[0].name !== actualValue.name) {
        ...
        return {
            pass: false, message: 'except error type and message are incorrect.'
        };
    }

    if (expectedValue.length > 2) {
        return {
            pass: false, message: 'Up to two parameters are supported.'
        };
    }
}
